...
_wp = [_unitG,_DefPos,_tp,"AWARE","YELLOW","FULL",["true","deletewaypoint [(group this), 0];"],true,0,[0,0,0],_frm] call RYD_WPadd;
...
waituntil
	...
	*/
		...
		_VLdr = vehicle (leader _x);
		_dstAct = _VLdr distance _HQpos;
		if (_dstAct > _sumD) then {_sumD = _dstAct};
		}
	foreach (_HQ getVariable ["RydHQ_Friends",[]]);

	if (_sumD > 0) then {_dstAv = _sumD};
	...
	_HQchance = (100 - (_HQsupporting * 20))/(1 + ((_HQ getVariable ["RydHQ_Recklessness",0.5]) * 0.5));
	if (_HQchance < 15) then {_HQchance = 15};
	...
	if ((_HQinDanger) and ((random 100) < _HQchance)) then
		...
		if ((_dstC > 2500) or (_nE isKindOf "Air")) then {_nE = ObjNull};
		...
		if (not (isNull _potential) and (({alive _x} count (units _potential)) > 0)) then
			...
			if ((random 0.5) < _reinfChance) then
				{
	...
	if not (isNull _chosen) then 
		...
		if not (_task isEqualTo taskNull) then
			...
			[_task,(leader _unitG),["Reinforce designated defensive position.", "Reinforce Defensive Position", ""],_RnfP,"ASSIGNED",0,false,true] call BIS_fnc_SetTask;
		...
		if ((_HQ getVariable ["RydHQ_Debug",false]) or (isPlayer (leader _unitG))) then 
			{
			_signum = _HQ getVariable ["RydHQ_CodeSign","X"];
			_i = [_RnfP,_unitG,"markReinf","ColorRed","ICON","waypoint","DRFP " + (groupId _unitG) + " " + _signum," - REINFORCE POSITION",[0.3,0.3]] call RYD_Mark
			}
		};
	...
	if (_unitG getVariable ["Unable",false]) then {_alive = false; _unitG setVariable [("Busy" + (str _unitG)),false]; _unitG setVariable ["Defending", false]};

	if (((vehicle (leader _unitG)) distance _DefPos) < 500) then {_endThis = true;};
	if not (isNil "_RnfP") then {if (((vehicle (leader _unitG)) distance _RnfP) < 500) then {_endThis = true;};};
