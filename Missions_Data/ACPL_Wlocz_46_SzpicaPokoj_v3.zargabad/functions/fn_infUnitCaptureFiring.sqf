...
_unit addEventHandler ["Fired", {BIS_Capture_Fired = true; BIS_Capture_FiredClass = (_this select 1);BIS_Capture_FiredMode = (_this select 3);}];
...
while { ((time <= _timeFinal) && (alive _unit) && !(BIS_EndCapture)) } do
{
	if (BIS_Capture_Fired) then
	{
		...
		{
			_capturedFireData = _capturedFireData + [[(_timeCur + _timeOffset), BIS_Capture_FiredClass, BIS_Capture_FiredMode, _unitTarget]];
		}
		else
		{
			_capturedFireData = _capturedFireData + [[(_timeCur + _timeOffset), BIS_Capture_FiredClass, BIS_Capture_FiredMode]];
		};
	};
	_frameTime = time;
