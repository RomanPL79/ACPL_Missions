RYD_StatusQuo = 
	...
	_RydMarks = _HQ getvariable ["RydMarks",[]];
	{deleteMarker _x} foreach _RydMarks;
	_RydMarks = [];
	_MarkGrps = ((_HQ getVariable ["RydHQ_KnEnemiesG",[]]) + (_HQ getVariable ["RydHQ_Friends",[]]) - _IgnoredPLY);
	_HQ setvariable ["RydMarksDebug",_MarkGrps];
		...
		//	_ply = _x;
			{
				...
				_mrk = createMarker ["mark" + (groupId _x),(leader _x)];
				_mrkcolor = format ["Color%1", side _x];
				_mrktype = _x call HAL_fnc_getType;
				_mrksize = [_x,units _x,_mrktype] call HAL_fnc_getSize;

				if ((side _x) == WEST) then {_mrktype = "b_" + _mrktype};
				if ((side _x) == EAST) then {_mrktype = "o_" + _mrktype};
				if ((side _x) == RESISTANCE) then {_mrktype = "i_" + _mrktype};
				_mrk setMarkerType _mrktype;
				_mrk setMarkerColor _mrkcolor;

				if not (_mrksize == -1) then {

					_mrk2 = createMarker ["mark2" + (groupId _x),(leader _x)];
...
/*
				...
				if (((side _x) == (side _HQ)) and not ((currentWaypoint _x) == 0)) then {

					_distance = (leader _x) distance (waypointPosition [_x,currentWaypoint _x]);
					_dX = ((waypointPosition [_x,currentWaypoint _x]) select 0) - ((position (leader _x)) select 0);
					_dY = ((waypointPosition [_x,currentWaypoint _x]) select 1) - ((position (leader _x)) select 1);
					_angle = _dX atan2 _dY;
					if (_angle >= 180) then {_angle = _angle - 180};
					_dXb = (_distance/2) * (sin _angle);
					_dYb = (_distance/2) * (cos _angle);
					_posX = ((position (leader _x)) select 0) + _dXb;
					_posY = ((position (leader _x)) select 1) + _dYb;

					_mrk3 = "destmrk" + str (_x);
					_mrk3 = createMarker [_mrk3,[_posX,_posY]];
					_mrk3 setMarkerColor "ColorBlack";
					_mrk3 setMarkerShape "RECTANGLE";
					_mrk3 setMarkerSize [0.5,_distance/2];
					_mrk3 setMarkerBrush "Solid";
					_mrk3 setMarkerdir _angle;
				};
*/
				_RydMarks pushBack _mrk;
				if not (_mrksize == -1) then {_RydMarks pushBack _mrk2};
//				if not (isNil "_mrk3") then {_RydMarks pushBack _mrk3};			
						
			} foreach _MarkGrps;
		//} foreach _SidePLY;

		_HQ setvariable ["RydMarks",_RydMarks];
	};
	...
	if (((_moraleInfl > _enemyInfl) and not (isNil "_obj") and not ((count _objs) < 1) and {not ((_HQ getVariable ["RydHQ_Order","ATTACK"]) in ["DEFEND"])}) or {(_HQ getVariable ["RydHQ_Berserk",false])} or {(_moraleInfl > _enemyInfl) and (((75)*(_HQ getVariable ["RydHQ_Recklessness",0.5])*(count (_HQ getVariable ["RydHQ_KnEnemiesG",[]]))) >= (random 100))}) then 
		{
