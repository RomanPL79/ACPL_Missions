...
RYD_Wait = 
	...
	"_isInside","_isOutside","_enG","_arr","_type","_cplR","_cWp","_wpCheck","_boxed","_firedF","_fCount","_forBoxing","_wp","_pass","_Unable","_isPlayer"];
	...
	waituntil 
		...
		if (_alive) then
			...
			_Unable = _gp getvariable "Unable";
			if (isNil "_Unable") then {_Unable = false};
		...
		((((count (waypoints _GDV)) < _wplimit) and (_wpCheck)) or ((_timer > _tolerance) and not (_isPlayer)) or ((_enemy) and not (_isPlayer)) or (_Unable) or not (_alive) or (_isInside) or (_isOutside) or (_busy))
		};
...
RYD_AddTask = 
	...
	if (_presentplayer) then
		...
		_task = [(group _unit),"title", _descr, _dstn,"ASSIGNED",0,true,_type,true] call BIS_fnc_taskCreate;
		_tasks = [];
