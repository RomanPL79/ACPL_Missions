...
acpl_loop = {
	private ["_playable", "_static", "_cities"];
	...
	acpl_medical_healing = [];
	publicvariable "acpl_medical_healing";
	
	acpl_civilian_shootingpos = [];
	
	acpl_cities = [];
	
	_cities = nearestLocations [[0,0,0], ["NameCity", "NameVillage", "NameCityCapital"], 100000];
	
	{
		private ["_pos", "_text", "_buildings", "_checked_buildings"];
		
		_pos = locationPosition _x;
		_text = text _x;
		_text = toupper(_text);
		_buildings = nearestObjects [_pos, ["house", "Building"], acpl_civs_locationradius];
		...
		acpl_cities = acpl_cities + [[_pos, _text, _checked_buildings]];
	} foreach _cities;
		
	_playable = [] + playableUnits + switchableUnits + allPlayers;
	
	if (_static) then {{[_x,"move"] remoteExec ["disableAI",0];} foreach _playable;};
	...
	while {true} do {
		...
		if (acpl_loop_slower) then {sleep 0.1;};
		{
			if (_x in acpl_variables_done) then {} else {
				...
				_x setvariable ["acpl_heal_dragging",false,true];
				_x setvariable ["acpl_heal_group",(group _x),true];
				...
				if (isNil {_x getVariable "acpl_ied_active"}) then {_x setvariable ["acpl_ied_active",[],true];};
				if (isNil {_x getVariable "acpl_ied_civ"}) then {_x setvariable ["acpl_ied_civ",false,true];};
				
				_x setvariable ["acpl_civilian_simulated", false, true];
				_x setvariable ["acpl_civilian_town", [], true];
				_x setvariable ["acpl_civilian_home", [], true];
				_x setvariable ["acpl_civilian_panicking", false, true];
				_x setvariable ["acpl_civilian_panicking_time", 0, true];
				_x setvariable ["acpl_civilian_panicking_inside", false, true];
				if (isNil {_x getVariable "acpl_civilian_excluded"}) then {_x setvariable ["acpl_civilian_excluded", false, true];};
				_x setvariable ["acpl_civilian_car", [], true];
				
				_x setvariable ["acpl_heal_side", side _x, true];
			...
			if (!(_x getVariable "acpl_civilian_excluded") AND (side _x == civilian) AND !(_x getvariable "ace_captives_ishandcuffed") AND !(_x getVariable "acpl_ied_civ") AND (count (_x getvariable "acpl_ied_active") == 0) AND !(captive _x)) then {
				[_x] spawn acpl_civ_simulation;
				[_x] spawn acpl_event_shot;
				[_x] call acpl_medic_remove;
			};
