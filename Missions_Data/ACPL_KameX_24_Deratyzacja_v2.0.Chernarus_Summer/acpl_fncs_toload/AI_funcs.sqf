...
acpl_drop_customweapon = {
	...
	waitUntil {sleep 0.01;!(_class in weapons _dummy)};
...
acpl_moveto = {
	...
	if (isNil "_z") then {_pos = [(_pos select 0),(_pos select 1),(getTerrainHeightASL _pos)];};
	...
	while {_onmove} do {
		if (!(isNil "_unit2")) then {_pos = getposATL _unit2;};
		if ((_unit distance _pos < 1) OR (_unit getvariable "acpl_moveto_completed")) then {
			_onmove = false;
			_unit setvariable ["acpl_moveto_completed",true,true];
		};
		
		if (((_unit distance _pos) < 5) AND ((_lastpos distance _unit) < 1)) then {
			_unit setvariable ["acpl_moveto_completed",true,true];
			_unit setposATL _pos;
		};
		
		if (!(currentCommand _unit == "MOVE")) then {[_unit,_pos] remoteExec ["domove",0];};
