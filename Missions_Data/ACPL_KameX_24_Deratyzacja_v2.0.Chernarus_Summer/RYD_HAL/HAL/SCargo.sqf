...
//	if (_request) then {_GD setvariable ["HAL_ReqTraOn",false,true]};
	...
	if not (_request) then {
		...
		_wp = [_GD,_Lpos,"MOVE","CARELESS","YELLOW","FULL",["true","{(vehicle _x) land 'GET IN'} foreach (units (group this));deletewaypoint [(group this), 0];"],true,0,[0,0,0],"COLUMN"] call RYD_WPadd;

	} else {

		_wp = [_GD,_Lpos,"MOVE","CARELESS","YELLOW","FULL",["true","{(vehicle _x) land 'LAND'} foreach (units (group this));deletewaypoint [(group this), 0];"],true,0,[0,0,0],"COLUMN"] call RYD_WPadd;
	...
	_GD setVariable ["zbe_cacheDisabled",true];
	_unitG setVariable ["zbe_cacheDisabled",true];
	...
	if (_ct > 300) then {_alive = false;_unitG leaveVehicle _ChosenOne;_unitG setVariable [("CC" + _unitvar), true]};
	};
...
if not (_request) then {_unitG setVariable [("CC" + _unitvar), true]};
...
//				[(leader _unitG),""] remoteExecCall ["onMapSingleClick",(leader _unitG)];
		...
		(not (_busy) or (_timer > 120) or (_reqdone) or not (_alive));
		};
...
//		_GD setvariable ["HAL_ReqTraOn",true,true];
	...
	if ((_timer > 120) or (_request)) then 
		{
	...
	_GD setVariable ["zbe_cacheDisabled",false];
	_unitG setVariable ["zbe_cacheDisabled",false];

	_GD = (group (assigneddriver _ChosenOne));

	_beh = "AWARE";
	if (_unitG in (_HQ getVariable ["RydHQ_RAirG",[]])) then {_beh = "CARELESS"};
	_wp = [_GD,_LandPos,"MOVE",_beh,"YELLOW","FULL",["true","{(vehicle _x) land 'LAND'} foreach (units (group this));deletewaypoint [(group this), 0]"],true,0,[0,0,0],"COLUMN"] call RYD_WPadd;
