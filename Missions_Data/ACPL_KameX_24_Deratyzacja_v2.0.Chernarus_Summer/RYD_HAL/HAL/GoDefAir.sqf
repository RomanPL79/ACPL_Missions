...
if (_busy) then 
	...
	waitUntil
		...
		switch (true) do
			...
			case (_unitG getVariable ["Unable",false]) : {_alive = false; _unitG setVariable [("Busy" + (str _unitG)),false]; _unitG setVariable ["Defending", false]}
			};
...
_wp = [_unitG,_StartPos,"MOVE","CARELESS","GREEN","NORMAL",["true", "{(vehicle _x) land 'LAND'} foreach (units (group this)); deletewaypoint [(group this), 0]"]] call RYD_WPadd;
