/*
	...
	a)	_capturedAnimData = [[2.135,"GAU8","<NULL-object>"]];
		[BIS_Vehicle, _capturedData, true] spawn BIS_fnc_UnitPlayFiring;
			Which would play back the captured data in the _capturedAnimData array (containing only one frame in this example)
...
private ["_targetUnit","_capturedAnimData","_totalFrames","_startedTime","_currentTime","_endSeq","_ignoreDisabled","_tmpTime","_tmpAnim","_tmpTarget"];

_targetUnit = _this select 0;

_capturedAnimData = _this select 1;
...

{
	if !(_endSeq) then
	{
		...
		[_targetUnit,_tmpAnim] remoteExec ["playMoveNow",0];
		if ((AnimationState _targetUnit) == _tmpAnim) then {
			[_targetUnit,_tmpAnim] remoteExec ["switchmove",0];
		};
