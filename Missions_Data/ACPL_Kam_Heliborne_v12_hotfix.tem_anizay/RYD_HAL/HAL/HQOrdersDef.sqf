...
switch ((random 100) >= (50/(0.5 + (_HQ getVariable ["RydHQ_Fineness",0.5])))) do
	...
	case true : 
				...
				if (({alive _x} count (units _x)) > 0) then
					...
					if (not (_isDef) and not (_Unable)) then
						...
						if not (_x in (_HQ getVariable ["RydHQ_Garrison",[]])) then
							...
							if (_ammo > 0) then
								{
								if not ((_x getVariable ["Busy" + (str _x),false]) and (_x in (_HQ getVariable ["RydHQ_DefSpot",[]]))) then
									{
									_closestArr = _defArray select 0;
									_friend =  vehicle (leader _x);
									_dstM = _friend distance (_closestArr select 0);
		...
		foreach ((_LMCU - ((_HQ getVariable ["RydHQ_DefSpot",[]]) + (_HQ getVariable ["RydHQ_Exhausted",[]]) + (_HQ getVariable ["RydHQ_RecDefSpot",[]]) + (_HQ getVariable ["RydHQ_DefRes",[]])) + (_HQ getVariable ["RydHQ_NCCargoG",[]]) + (_HQ getVariable ["RydHQ_SupportG",[]])) - (_HQ getVariable ["RydHQ_NoDef",[]]));
		};
	...
	case false : 
		...
		foreach ((_LMCU - ((_HQ getVariable ["RydHQ_RecDefSpot",[]]) + (_HQ getVariable ["RydHQ_Exhausted",[]]) + (_HQ getVariable ["RydHQ_DefRes",[]])) + (_HQ getVariable ["RydHQ_NCCargoG",[]]) + (_HQ getVariable ["RydHQ_SupportG",[]])) - (_HQ getVariable ["RydHQ_NoDef",[]]));
