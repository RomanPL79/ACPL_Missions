...
if (((_HQ getVariable ["RydHQ_CargoFind",0]) > 0) and not (_IsAPlayer)) then 
	...
	waituntil 
		...
		switch (true) do
			...
			case (_unitG getVariable ["Unable",false]) : {_alive = false; _unitG setVariable [("Busy" + (str _unitG)),false]; _unitG setVariable [("Capt" + (str _unitG)),false]};
			};
...
if (not (isNull _AV) and ((_HQ getVariable ["RydHQ_CargoFind",0]) > 0) and not (_GDV == _unitG) and not (_IsAPlayer)) then
	{
	_task = [(leader _unitG),["Embark your lift to the objective.", "Get In Lift", ""],(getPosATL (leader _unitG)),"getin"] call RYD_AddTask;
...
if (not ((leader _GDV) == (leader _unitG)) and not (_GDV == _unitG)) then
	{
	_Ctask = [(leader _GDV),["Disembark group at  their designated destination.", "Provide Lift", ""],[_dropposX,_dropposY],"getout"] call RYD_AddTask
	};

_gp = _unitG;
if not (isNull _AV) then {_gp = _GDV;_posX = _dropposX;_posY = _dropposY};
_pos = [_posX,_posY];
_tp = "MOVE";
