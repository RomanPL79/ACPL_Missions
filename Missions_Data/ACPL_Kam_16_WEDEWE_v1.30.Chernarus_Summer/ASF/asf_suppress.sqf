...

	{

		{
			if (alive _x) then {
				...
				//x1_debug groupChat (format ["%1, %2, %3, %4, %5",(typeName _x),_asf_units,_knowledge,_posArray,currentCommand _x]);
...

		{
			if (canFire _x) then { 
				...
				_knowledge = (_x knowsAbout (_posArray select 0));
				x1_debug groupChat (format ["%1, %2, %3, %4, %5",(typeName _x),_asf_vehicles,_knowledge,_posArray,currentCommand gunner _x]);				
			} else {
