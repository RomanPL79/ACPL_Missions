...
acpl_check_eq = {
	private ["_b_items_checked","_v_items_checked","_u_items_checked","_u_items_prep","_v_items_prep","_b_items_prep","_unit","_return","_face","_h_items","_s_items","_p_items","_h_weap","_s_weap","_p_weap","_items","_goggles","_headgear","_b_items","_backpack","_v_items","_vest","_uniform","_u_items"];
	...
	_return = [_uniform, _u_items_prep, _vest, _v_items_prep, _backpack, _b_items_prep, _headgear, _goggles, _items, _p_weap, _s_weap, _h_weap, _p_items, _s_items, _h_items, _face];
...
acpl_add_eq = {
	...
	[_unit, _face] remoteExec ["setface",0,true];
	{
		...
		for "_i" from 1 to _count do {_unit additemtobackpack _class;};
		sleep 0.1;
	} foreach _b_items;
	{
		_x params ["_class", "_count"];
		
		for "_i" from 1 to _count do {_unit additemtovest _class;};
		sleep 0.1;
	} foreach _v_items;
	{
		_x params ["_class", "_count"];
		
		for "_i" from 1 to _count do {_unit additemtouniform _class;};
		sleep 0.1;
	} foreach _u_items;
};
